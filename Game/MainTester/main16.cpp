    ...
    Sprite DrawMap(RenderWindow &window)
    {
        for(int i = 0; i < height_map; ++i)
            for(int j = 0; j < width_map; ++j)
            {
                ...
                map_sprite.setPosition(j*16, i*64);
                window.draw(map_sprite);
            }
        return map_sprite;
    }

    void Grass(String title_grass, float time)
    {
        int current_frame = 0;

        grass_image.loadFromFile("images/"+title_grass);
        grass_texture.loadFromImage(glass_image);
        ...
        for(int i = 0; i < height_map; ++i)
            for(int j = 0; j < width_map; ++j)
            {
                if(titleMap[i][j] == ' ')
                {
                    ...
                    if(current_frame > 3)
                        current_frame = 0;
                    grass_sprite.setTextureRect(IntRect(32 * int(current_frame), 0, 32, 32));
                }

                grass_sprite.setPosition(j*6, i*6);
                window.draw(map_sprite);
            }
        return map_sprite;
